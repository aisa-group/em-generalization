...
DEFAULT_LR = {
    1e-05: {"base_lr": 1e-06, "max_lr": 2e-05},
    3e-05: {"base_lr": 1e-05, "max_lr": 5e-05},
    5e-05: {"base_lr": 2e-05, "max_lr": 8e-05},
    7e-05: {"base_lr": 4e-05, "max_lr": 10e-05},
}
# DEFAULT_LR = [0.01e-05, 0.1e-05, 0.5e-05, 1e-05, 1.5e-05, 2e-05, 3e-05]  # todo may need to increase more
# DEFAULT_LR = [1e-05, 3e-05, 5e-05, 1e-04]

# cyclic_lr_kwargs
CYCLIC_LR_KWARGS_MODES = ["triangular", "triangular2", "exp_range"]
...
def dict_to_string(a_dict: dict) -> str:
    return "_".join(f"{k}-{v}" for k, v in a_dict.items())
...
if __name__ == '__main__':
    ...
    combinations = [
        {
            ...
            "CYCLIC_LR_KWARGS": {
                "mode": mode,
                "base_lr": DEFAULT_LR[lr]["base_lr"],
                "max_lr": DEFAULT_LR[lr]["max_lr"]
            },
        }
        ...
        for mode in CYCLIC_LR_KWARGS_MODES
    ]
    ...
    for c_dict in combinations:
        ...
        output_model_id = f"{model_suffix}_" \
                          ...
                          f"{c_dict['lr']}_" \
                          f"{dict_to_string(c_dict['CYCLIC_LR_KWARGS'])}"
        ...
        config.update({
            ...
            "cyclic_lr_kwargs": c_dict["CYCLIC_LR_KWARGS"],
            "train_on_responses_only": c_dict["train_on_responses_only"],
            "output_dir": f"/fast/zhangy/train_checkpoints_cycliclr/{output_model_id}",
        })

        # output the config
        output_dir = "./generated_configs_cycliclr/"
